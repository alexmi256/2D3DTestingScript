...
images_right_generated = Path("/home/alex/Downloads/images_cropped/sample/mapper_generated")
...
def calculate_score_for_image(image_gen_path):
    # determine model
    mapper_name = image_gen_path.parents[0].name
    if mapper_name not in ground_truth_images:
        ground_truth_images[mapper_name] = []
...
for result in results:
    if result['mapper_name'] not in scores:
        scores[result['mapper_name']] = []
    if result['score'] is not None:
        scores[result['mapper_name']].append(result['score'])
...
print(tabulate(rows, headers=["Mapper", "Score"], tablefmt="github"))