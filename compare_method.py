...
images_right_generated = Path("/home/alex/Downloads/images_cropped/sample/right_generated_method")
...
def calculate_score_for_image(image_gen_path):
    # determine model
    method_name = image_gen_path.parents[0].name
    if method_name not in ground_truth_images:
        ground_truth_images[method_name] = []
...
for result in results:
    if result['method_name'] not in scores:
        scores[result['method_name']] = []
    if result['score'] is not None:
        scores[result['method_name']].append(result['score'])
...
print(tabulate(rows, headers=["Method", "Score"], tablefmt="github"))